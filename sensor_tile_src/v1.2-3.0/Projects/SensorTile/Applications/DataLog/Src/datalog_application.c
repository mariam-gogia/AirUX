...
void Gyro_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
      ...
      uint32_t abs_ang_vel;
      abs_ang_vel = ((int)angular_velocity.AXIS_X * (int)angular_velocity.AXIS_X);
      abs_ang_vel += ((int)angular_velocity.AXIS_Y * (int)angular_velocity.AXIS_Y);
      abs_ang_vel += ((int)angular_velocity.AXIS_Z * (int)angular_velocity.AXIS_Z);
      abs_ang_vel = sqrt((float) abs_ang_vel);


      sprintf( dataOut, "\n\rGYR_X: %d, GYR_Y: %d, GYR_Z: %d, |VEL|: %d",
